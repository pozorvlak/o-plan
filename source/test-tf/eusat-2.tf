...
;;;                ------------------------------------------    
;;;
;;; RBK  9-Dec-91 : Adjusted to load into O-Plan2.
...
;;;                 conditions;
;;; BD   7-May-92 : Moved to ~oplan2/release/demonstrations/tf as base file 
;;;                 for release
...
;;;-----------------
  ...
  ;;; no orderings
  ;;; /\/: Added conds to protect from the actual effects to the
  ;;; end of this node, because the parent's conds protect only
  ;;; from the end of this node, not from the actual effects.
  ;;; [jd, June 1999]
  conditions supervised {input switch4} = ?line 
                at end_of self from [1],
	     supervised {mode ground_buffer} = write
                at end_of self from [4];
end_schema;
...
;;;
  ...
  conditions only_use_for_query {switch_input_can_be ?sw ?x},
             ...
             ;;; flexible handling is allowed by the O-Plan2 implementation
end_schema;
